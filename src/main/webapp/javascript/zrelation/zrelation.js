var zrelation = function() {
	$("#searchzrelations").jqxMenu({
		theme : "base",
		width : "100%",
		height : 35
	});
...
zrelation.prototype = {
	 ...
	 * 初始化数据源的列属性
	 */
	sourceDatafields : [{name : 'zNote',type : 'string'},{name : 'zQuietStart',type : 'int'},{name : 'zBUserId',type : 'int'},{name : 'zStatus',type : 'int'},{name : 'zSentFrequent',type : 'int'},{name : 'zAUserId',type : 'int'},{name : 'zRequestTimes',type : 'int'},{name : 'zRelation',type : 'int'},{name : 'zQuietStop',type : 'int'}],
	/**
	 * 初始化Grid的列属性
	 */
	gridColumns : [ {text : 'zrelation zNote',datafield : 'zNote',width : '20%'},{text : 'zrelation zQuietStart',datafield : 'zQuietStart',width : '20%'},{text : 'zrelation zBUserId',datafield : 'zBUserId',width : '20%'},{text : 'zrelation zStatus',datafield : 'zStatus',width : '20%'},{text : 'zrelation zSentFrequent',datafield : 'zSentFrequent',width : '20%'},{text : 'zrelation zAUserId',datafield : 'zAUserId',width : '20%'},{text : 'zrelation zRequestTimes',datafield : 'zRequestTimes',width : '20%'},{text : 'zrelation zRelation',datafield : 'zRelation',width : '20%'},{text : 'zrelation zQuietStop',datafield : 'zQuietStop',width : '20%'} ],
	/**
	 * 初始化数据源
	 * 
	 * @param userName
	...
	initGrid : function(id, dataAdapter, columns) {
		$("#" + id).jqxGrid({
			...
			rendertoolbar : function(toolbar) {
				zrelation.prototype.render(toolbar);
			},
			columns : columns
		});
	},
	...
	render : function(toolbar) {
		...
		addButton.click(function(event) {
			zrelation.prototype.emptyForm(1);
			zrelation.prototype.openWindows("当前位置：用户管理&gt&gt新增用户");
		});
		editButton.click(function() {
			var selectedRowIndex = $("#zrelationManagerGrid").jqxGrid('getselectedrowindexes');
			if (selectedRowIndex.length == 1) {
				var id = $("#zrelationManagerGrid").jqxGrid('getrowdata',selectedRowIndex[0]);
				$.get("./action/zrelation/getzrelation/"+id.id, 
					function(data) {
					
						$.each(data, function(name, val){
						    var $el = $('#zrelation_info_form input[id="'+name+'"]'),
						        type = $el.attr('type');	
						    switch(type){
						        case 'checkbox':
					...
					zrelation.prototype.initInputButtonAndText('#zrelation_info_form');
					zrelation.prototype.validator("#zrelation_info_form");
					zrelation.prototype.openWindows("当前位置：用户管理&gt&gt修改用户信息");
				});
			} else {
				alert("请选择一条记录");
			}
		...
		deleteButton.click(function(event) {
			...
			if (confirm("确认要删除吗？")) {
				for ( var i = 0; i < selectedRowIndex.length; i++) {
					var id = $("#zrelationManagerGrid").jqxGrid('getrowdata',selectedRowIndex[i]);
					selectionObj.push(id.id);
				}
				zrelation.prototype.delReports(selectionObj);
			}
		});
	},
	/**
	...
	usersSearch : function() {
		$("#searchbutton").on('click', function(data) {
		    $("zrelationManagerGrid").jqxGrid('updatebounddata');
	    });
	},
	/**
	 * 删除记录
	...
	delReports : function(usersId) {
		var url = "./action/zrelation/delzrelations";
		var success = function(data) {
			$('#zrelationManagerGrid').jqxGrid('updatebounddata');
			$('#zrelationManagerGrid').jqxGrid('clearselection');
		};
		zrelation.prototype.ajax("POST", "json", {
			"zrelationsId" : usersId
		}, url, success);
	},
	...
	openWindows : function(str) {
		...
		$('#savebutton').on('click', function(event) {
			// var result = $('#zrelation_info_form').jqxValidator('validate');
			// if (result) {
			zrelation.prototype.saveORUpdate();
			// }
		});
	...
	saveORUpdate : function() {
		...
		var success = function(data) {
			if (confirm(data.message)) {
				window.location = "./action/zrelation/index";
			}
		};
		zrelation.prototype.ajax("POST", "json", data, url, success);
	},
	/**
	...
	gameOn : function(id) {
		var url = "./action/zrelation/getzrelations?start=0&end=10";
		zrelation.prototype.initGrid("zrelationManagerGrid",
				zrelation.prototype.initSource("",zrelation.prototype.sourceDatafields, url),
				zrelation.prototype.gridColumns);
	}
};

$(function() {
	var users = new zrelation();
	try {
		users.initInputButtonAndText("#searchzrelations,#addUpdateManagerWindowContent");
		users.validator("#zrelation_info_form");
		users.gameOn('');
		users.usersSearch();
	} catch (e) {
