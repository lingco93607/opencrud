...
zgroup.prototype = {
	 ...
	 * 初始化数据源的列属性
	 */
	sourceDatafields : [{name : 'zGroupLevel',type : 'int'},{name : 'zGroupId',type : 'int'},{name : 'zGroupParent',type : 'int'},{name : 'zGroupName',type : 'int'}],
	/**
	 * 初始化Grid的列属性
	 */
	gridColumns : [ {text : 'zgroup zGroupLevel',datafield : 'zGroupLevel',width : '20%'},{text : 'zgroup zGroupId',datafield : 'zGroupId',width : '20%'},{text : 'zgroup zGroupParent',datafield : 'zGroupParent',width : '20%'},{text : 'zgroup zGroupName',datafield : 'zGroupName',width : '20%'} ],
	/**
	 * 初始化数据源
	 * 
	 * @param userName
	...
	initGrid : function(id, dataAdapter, columns) {
		$("#" + id).jqxGrid({
			...
			rendertoolbar : function(toolbar) {
				zgroup.prototype.render(toolbar);
			},
			columns : columns
		});
	},
	...
	render : function(toolbar) {
		...
		addButton.click(function(event) {
			zgroup.prototype.emptyForm(1);
			zgroup.prototype.openWindows("当前位置：用户管理&gt&gt新增用户");
		});
		editButton.click(function() {
			var selectedRowIndex = $("#zgroupManagerGrid").jqxGrid('getselectedrowindexes');
			if (selectedRowIndex.length == 1) {
				var id = $("#zgroupManagerGrid").jqxGrid('getrowdata',selectedRowIndex[0]);
				$.get("./action/zgroup/getzgroup/"+id.id, 
					function(data) {
					
						$.each(data, function(name, val){
						    var $el = $('#zgroup_info_form input[id="'+name+'"]'),
						        type = $el.attr('type');	
						    switch(type){
						        case 'checkbox':
					...
					zgroup.prototype.initInputButtonAndText('#zgroup_info_form');
					zgroup.prototype.validator("#zgroup_info_form");
					zgroup.prototype.openWindows("当前位置：用户管理&gt&gt修改用户信息");
				});
			} else {
				alert("请选择一条记录");
			}
		...
		deleteButton.click(function(event) {
			...
			if (confirm("确认要删除吗？")) {
				for ( var i = 0; i < selectedRowIndex.length; i++) {
					var id = $("#zgroupManagerGrid").jqxGrid('getrowdata',selectedRowIndex[i]);
					selectionObj.push(id.id);
				}
				zgroup.prototype.delReports(selectionObj);
			}
		});
	},
	/**
	...
	usersSearch : function() {
		$("#searchbutton").on('click', function(data) {
		    $("zgroupManagerGrid").jqxGrid('updatebounddata');
	    });
	},
	/**
	 * 删除记录
	...
	delReports : function(usersId) {
		var url = "./action/zgroup/delzgroups";
		var success = function(data) {
			$('#zgroupManagerGrid').jqxGrid('updatebounddata');
			$('#zgroupManagerGrid').jqxGrid('clearselection');
		};
		zgroup.prototype.ajax("POST", "json", {
			"zgroupsId" : usersId
		}, url, success);
	},
	...
	openWindows : function(str) {
		...
		$('#savebutton').on('click', function(event) {
			// var result = $('#zgroup_info_form').jqxValidator('validate');
			// if (result) {
			zgroup.prototype.saveORUpdate();
			// }
		});
	...
	saveORUpdate : function() {
		...
		var success = function(data) {
			if (confirm(data.message)) {
				window.location = "./action/zgroup/index";
			}
		};
		zgroup.prototype.ajax("POST", "json", data, url, success);
	},
	/**
	...
	gameOn : function(id) {
		var url = "./action/zgroup/getzgroups?start=0&end=10";
		zgroup.prototype.initGrid("zgroupManagerGrid",
				zgroup.prototype.initSource("",zgroup.prototype.sourceDatafields, url),
				zgroup.prototype.gridColumns);
	}
};

$(function() {
	var users = new zgroup();
	try {
		users.initInputButtonAndText("#searchzgroups,#addUpdateManagerWindowContent");
		users.validator("#zgroup_info_form");
		users.gameOn('');
		users.usersSearch();
	} catch (e) {
