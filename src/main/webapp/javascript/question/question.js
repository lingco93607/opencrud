...
question.prototype = {
	 ...
	 * 初始化数据源的列属性
	 */
	sourceDatafields : [{name : 'questionID',type : 'int'},{name : 'readNum',type : 'int'},{name : 'text',type : 'string'},{name : 'authorID',type : 'int'},{name : 'status',type : 'int'}],
	/**
	 * 初始化Grid的列属性
	 */
	gridColumns : [ {text : 'question questionID',datafield : 'questionID',width : '20%'},{text : 'question readNum',datafield : 'readNum',width : '20%'},{text : 'question text',datafield : 'text',width : '20%'},{text : 'question authorID',datafield : 'authorID',width : '20%'},{text : 'question status',datafield : 'status',width : '20%'} ],
	/**
	 * 初始化数据源
	 * 
	 * @param userName
	...
	initGrid : function(id, dataAdapter, columns) {
		$("#" + id).jqxGrid({
			...
			rendertoolbar : function(toolbar) {
				question.prototype.render(toolbar);
			},
			columns : columns
		});
	},
	...
	render : function(toolbar) {
		...
		addButton.click(function(event) {
			question.prototype.emptyForm(1);
			question.prototype.openWindows("当前位置：用户管理&gt&gt新增用户");
		});
		editButton.click(function() {
			var selectedRowIndex = $("#questionManagerGrid").jqxGrid('getselectedrowindexes');
			if (selectedRowIndex.length == 1) {
				var id = $("#questionManagerGrid").jqxGrid('getrowdata',selectedRowIndex[0]);
				$.get("./action/question/getquestion/"+id.id, 
					function(data) {
					
						$.each(data, function(name, val){
						    var $el = $('#question_info_form input[id="'+name+'"]'),
						        type = $el.attr('type');	
						    switch(type){
						        case 'checkbox':
					...
					question.prototype.initInputButtonAndText('#question_info_form');
					question.prototype.validator("#question_info_form");
					question.prototype.openWindows("当前位置：用户管理&gt&gt修改用户信息");
				});
			} else {
				alert("请选择一条记录");
			}
		...
		deleteButton.click(function(event) {
			...
			if (confirm("确认要删除吗？")) {
				for ( var i = 0; i < selectedRowIndex.length; i++) {
					var id = $("#questionManagerGrid").jqxGrid('getrowdata',selectedRowIndex[i]);
					selectionObj.push(id.id);
				}
				question.prototype.delReports(selectionObj);
			}
		});
	},
	/**
	...
	usersSearch : function() {
		$("#searchbutton").on('click', function(data) {
		    $("questionManagerGrid").jqxGrid('updatebounddata');
	    });
	},
	/**
	 * 删除记录
	...
	delReports : function(usersId) {
		var url = "./action/question/delquestions";
		var success = function(data) {
			$('#questionManagerGrid').jqxGrid('updatebounddata');
			$('#questionManagerGrid').jqxGrid('clearselection');
		};
		question.prototype.ajax("POST", "json", {
			"questionsId" : usersId
		}, url, success);
	},
	...
	openWindows : function(str) {
		...
		$('#savebutton').on('click', function(event) {
			// var result = $('#question_info_form').jqxValidator('validate');
			// if (result) {
			question.prototype.saveORUpdate();
			// }
		});
	...
	saveORUpdate : function() {
		...
		var success = function(data) {
			if (confirm(data.message)) {
				window.location = "./action/question/index";
			}
		};
		question.prototype.ajax("POST", "json", data, url, success);
	},
	/**
	...
	gameOn : function(id) {
		var url = "./action/question/getquestions?start=0&end=10";
		question.prototype.initGrid("questionManagerGrid",
				question.prototype.initSource("",question.prototype.sourceDatafields, url),
				question.prototype.gridColumns);
	}
};

$(function() {
	var users = new question();
	try {
		users.initInputButtonAndText("#searchquestions,#addUpdateManagerWindowContent");
		users.validator("#question_info_form");
		users.gameOn('');
		users.usersSearch();
	} catch (e) {
