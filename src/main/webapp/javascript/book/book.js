...
book.prototype = {
	 ...
	 * 初始化数据源的列属性
	 */
	sourceDatafields : [{name : 'name',type : 'string'},{name : 'id',type : 'int'}],
	/**
	 * 初始化Grid的列属性
	 */
	gridColumns : [ {text : 'book name',datafield : 'name',width : '20%'},{text : 'book id',datafield : 'id',width : '20%'} ],
	/**
	 * 初始化数据源
	 * 
	 * @param userName
	...
	render : function(toolbar) {
		...
		addButton.click(function(event) {
			book.prototype.emptyForm(1);
			book.prototype.openWindows("当前位置：用户管理&gt&gt新增用户");
		});
		editButton.click(function() {
			var selectedRowIndex = $("#bookManagerGrid").jqxGrid('getselectedrowindexes');
			if (selectedRowIndex.length == 1) {
				var id = $("#bookManagerGrid").jqxGrid('getrowdata',selectedRowIndex[0]);
				$.get("./action/book/getbook/"+id.id, 
					function(data) {
					
						$.each(data, function(name, val){
						    var $el = $('#book_info_form input[id="'+name+'"]'),
						        type = $el.attr('type');	
						    switch(type){
						        case 'checkbox':
					...
					book.prototype.initInputButtonAndText('#book_info_form');
					book.prototype.validator("#book_info_form");
					book.prototype.openWindows("当前位置：用户管理&gt&gt修改用户信息");
				});
			} else {
				alert("请选择一条记录");
			}
		...
		deleteButton.click(function(event) {
			...
			if (confirm("确认要删除吗？")) {
				for ( var i = 0; i < selectedRowIndex.length; i++) {
					var id = $("#bookManagerGrid").jqxGrid('getrowdata',selectedRowIndex[i]);
					selectionObj.push(id.id);
				}
				book.prototype.delReports(selectionObj);
			}
		});
	},
	/**
	...
	usersSearch : function() {
		$("#searchbutton").on('click', function(data) {
		    $("bookManagerGrid").jqxGrid('updatebounddata');
	    });
	},
	/**
	 * 删除记录
	...
	delReports : function(usersId) {
		var url = "./action/book/delbooks";
		var success = function(data) {
			$('#bookManagerGrid').jqxGrid('updatebounddata');
			$('#bookManagerGrid').jqxGrid('clearselection');
		};
		book.prototype.ajax("POST", "json", {
			"booksId" : usersId
		}, url, success);
	},
	...
	openWindows : function(str) {
		...
		$('#savebutton').on('click', function(event) {
			// var result = $('#book_info_form').jqxValidator('validate');
			// if (result) {
			book.prototype.saveORUpdate();
			// }
		});
	...
	saveORUpdate : function() {
		var data = $("#addUpdateManagerWindowContent").find("form")
				.serializeObject();
		var url = "./action/book/savebook";
		var success = function(data) {
			if (confirm(data.message)) {
				window.location = "./action/book/index";
			}
		};
		book.prototype.ajax("POST", "json", data, url, success);
	},
	/**
	...
	validator : function(id) {
		$(id).jqxValidator({
			rules : [ {input : '#name',message : 'name不能为空!',action : 'keyup, blur',rule : 'required'},{input : '#id',message : 'id不能为空!',action : 'keyup, blur',rule : 'required'} ]
		});
	},
	gameOn : function(id) {
		var url = "./action/book/getbooks?start=0&end=10";
		book.prototype.initGrid("bookManagerGrid",
				book.prototype.initSource("",book.prototype.sourceDatafields, url),
				book.prototype.gridColumns);
	}
};
...
$(function() {
	var users = new book();
	try {
		users.initInputButtonAndText("#searchbooks,#addUpdateManagerWindowContent");
		users.validator("#book_info_form");
		users.gameOn('');
		users.usersSearch();
	} catch (e) {
