var zusergroup = function() {
	$("#searchzusergroups").jqxMenu({
		theme : "base",
		width : "100%",
		height : 35
	});
...
zusergroup.prototype = {
	/**
	 * 初始化数据源的列属性
	 */
	sourceDatafields : [{name : 'zGroupId',type : 'int'},{name : 'zUserId',type : 'int'},{name : 'isAdministrator',type : 'int'},{name : 'isCreator',type : 'int'}],
	/**
	 * 初始化Grid的列属性
	 */
	gridColumns : [ {text : 'zusergroup zGroupId',datafield : 'zGroupId',width : '20%'},{text : 'zusergroup zUserId',datafield : 'zUserId',width : '20%'},{text : 'zusergroup isAdministrator',datafield : 'isAdministrator',width : '20%'},{text : 'zusergroup isCreator',datafield : 'isCreator',width : '20%'} ],
	/**
	 * 初始化数据源
	 * 
	 * @param userName
	...
	initGrid : function(id, dataAdapter, columns) {
		$("#" + id).jqxGrid({
			...
			rendertoolbar : function(toolbar) {
				zusergroup.prototype.render(toolbar);
			},
			columns : columns
		});
	},
	...
	render : function(toolbar) {
		...
		addButton.click(function(event) {
			zusergroup.prototype.emptyForm(1);
			zusergroup.prototype.openWindows("当前位置：用户管理&gt&gt新增用户");
		});
		editButton.click(function() {
			var selectedRowIndex = $("#zusergroupManagerGrid").jqxGrid('getselectedrowindexes');
			if (selectedRowIndex.length == 1) {
				var id = $("#zusergroupManagerGrid").jqxGrid('getrowdata',selectedRowIndex[0]);
				$.get("./action/zusergroup/getzusergroup/"+id.id, 
					function(data) {
					
						$.each(data, function(name, val){
						    var $el = $('#zusergroup_info_form input[id="'+name+'"]'),
						        type = $el.attr('type');	
						    switch(type){
						        case 'checkbox':
					...
					zusergroup.prototype.initInputButtonAndText('#zusergroup_info_form');
					zusergroup.prototype.validator("#zusergroup_info_form");
					zusergroup.prototype.openWindows("当前位置：用户管理&gt&gt修改用户信息");
				});
			} else {
				alert("请选择一条记录");
			}
		...
		deleteButton.click(function(event) {
			...
			if (confirm("确认要删除吗？")) {
				for ( var i = 0; i < selectedRowIndex.length; i++) {
					var id = $("#zusergroupManagerGrid").jqxGrid('getrowdata',selectedRowIndex[i]);
					selectionObj.push(id.id);
				}
				zusergroup.prototype.delReports(selectionObj);
			}
		});
	},
	/**
	...
	usersSearch : function() {
		$("#searchbutton").on('click', function(data) {
		    $("zusergroupManagerGrid").jqxGrid('updatebounddata');
	    });
	},
	/**
	 * 删除记录
	...
	delReports : function(usersId) {
		var url = "./action/zusergroup/delzusergroups";
		var success = function(data) {
			$('#zusergroupManagerGrid').jqxGrid('updatebounddata');
			$('#zusergroupManagerGrid').jqxGrid('clearselection');
		};
		zusergroup.prototype.ajax("POST", "json", {
			"zusergroupsId" : usersId
		}, url, success);
	},
	...
	openWindows : function(str) {
		...
		$('#savebutton').on('click', function(event) {
			// var result = $('#zusergroup_info_form').jqxValidator('validate');
			// if (result) {
			zusergroup.prototype.saveORUpdate();
			// }
		});
	...
	saveORUpdate : function() {
		...
		var success = function(data) {
			if (confirm(data.message)) {
				window.location = "./action/zusergroup/index";
			}
		};
		zusergroup.prototype.ajax("POST", "json", data, url, success);
	},
	/**
	...
	gameOn : function(id) {
		var url = "./action/zusergroup/getzusergroups?start=0&end=10";
		zusergroup.prototype.initGrid("zusergroupManagerGrid",
				zusergroup.prototype.initSource("",zusergroup.prototype.sourceDatafields, url),
				zusergroup.prototype.gridColumns);
	}
};

$(function() {
	var users = new zusergroup();
	try {
		users.initInputButtonAndText("#searchzusergroups,#addUpdateManagerWindowContent");
		users.validator("#zusergroup_info_form");
		users.gameOn('');
		users.usersSearch();
	} catch (e) {
