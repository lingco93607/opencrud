...
(function(w){
	...
	w[_namespace] = function(){
		...
		var _arg = arguments[0];
			...
			_historys = [],
			_placeHolder = _arg.placeHolder || '<p id="eleditorAddText" class="Eleditor-placeholder">点击此处编辑内容</p>';
		...
		if( _inArray('insertImage', _arg.toolbars) && typeof WebUploader != 'undefined' ){
			...
			_controllerUploader.on( 'uploadSuccess', function( _file, _call ) {
			  ...
			  //   	alert(_call.msg);
			  //   }
			  	//var imgUrl = "../../action/article/img/"+ _call._raw.split('.')[0] + "/" + _call._raw.split('.')[1];
			  	if(_call._raw != "") {
			  		var _$upImg = $('<img class="full-image" src="'+_call._raw+'">');
			  		_$selected.after(_$upImg);
					_appendHistory({ m: 'insertNode', node: _$upImg });
					$("#eleditorAddText").remove();
			  	}
			...
			_stylebarUploader.on( 'uploadSuccess', function( _file, _call ) {
				//var imgUrl = "../../action/article/img/"+ _call._raw.split('.')[0] + "/" + _call._raw.split('.')[1];
				if( _call._raw != "") {
					_$editorTextArea.append('<img class="full-image" src="'+imgUrl+'">');
					$("#eleditorAddText").remove();
				}
